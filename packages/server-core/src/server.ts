...
function getIps(): string[] {
  ...
  if (ips.length === 0) ips.push('192.168.43.1');
  return ips;
}
...
app.get('/api/debug/ips', (req, res) => {
  const ips = getIps();
  res.json({ ips, ip: ips[0] || '192.168.43.1', headers: req.headers, socket: (req.socket as any).localAddress });
});
...
httpServer.listen(PORT, HOST, () => {
  ...
  if (!staticDir) console.log('Tip: Run npm run build --workspace=web-client to enable SPA');
