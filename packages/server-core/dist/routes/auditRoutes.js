...
auditRouter.post('/audits', requireAuth, requireAdmin, (req, res) => {
    ...
    const ing = db.prepare(`SELECT current_stock FROM ingredients WHERE id=?`).get(ingredientId);
    if (!ing)
    ...
    try {
        ...
        if (e.message.includes('UNIQUE'))
            return res.status(409).json({ error: 'Audit already exists for this ingredient on this date' });
...
auditRouter.post('/audits/bulk', requireAuth, requireAdmin, (req, res) => {
    ...
    const tx = db.transaction(() => {
        for (const e of entries) {
            const ing = db.prepare(`SELECT current_stock FROM ingredients WHERE id=?`).get(e.ingredientId);
            if (!ing)
                throw new Error(`Ingredient ${e.ingredientId} not found`);
