...
export function createOrderRouter(wsGateway) {
    const router = Router();
    ...
    router.post('/orders', (req, res) => {
        ...
        for (const it of items) {
            const menuItem = db.prepare(`SELECT id, price, is_available, stall_id FROM menu_items WHERE id=?`).get(it.menuItemId);
            if (!menuItem)
                return res.status(404).json({ error: `Menu item ${it.menuItemId} not found` });
    ...
    router.get('/orders/:id', (req, res) => {
        const order = db.prepare(`SELECT o.*, s.name as stall_name FROM orders o LEFT JOIN stalls s ON s.id=o.stall_id WHERE o.id=?`).get(req.params.id);
        if (!order)
    ...
    router.get('/orders/by-code/:code', requireAuth, (req, res) => {
        ...
        const order = db.prepare(`SELECT * FROM orders WHERE pickup_code=?`).get(code);
        if (!order)
            return res.status(404).json({ error: 'Order not found for code ' + code });
        const user = getAuthUser(req);
    ...
    router.patch('/orders/:id/status', requireAuth, (req, res) => {
        ...
        try {
            ...
            if (shouldRollback) {
                try {
                    rollbackBomTransaction(order.id, mapped);
                }
                catch (e) {
