...
inventoryRouter.post('/recipe-bom', requireAuth, (req, res) => {
    ...
    const menuItem = db.prepare(`SELECT stall_id FROM menu_items WHERE id=?`).get(parsed.data.menuItemId);
    if (!menuItem)
        return res.status(404).json({ error: 'Menu item not found' });
    const user = getAuthUser(req);
    if (!user || !canManageStall(user, menuItem.stall_id))
...
inventoryRouter.delete('/recipe-bom/:id', requireAuth, (req, res) => {
    const existing = db.prepare(`SELECT rb.*, mi.stall_id FROM recipe_bom rb JOIN menu_items mi ON mi.id=rb.menu_item_id WHERE rb.id=?`).get(req.params.id);
    if (!existing)
        return res.status(404).json({ error: 'Not found' });
    const user = getAuthUser(req);
    if (!user || !canManageStall(user, existing.stall_id))
        return res.status(403).json({ error: 'Forbidden for this stall' });
