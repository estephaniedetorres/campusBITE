...
function upsertStall(id, name, desc, logoUrl, rating, ratingCount) {
    ...
    if (rating !== undefined) {
        try {
            db.prepare(`UPDATE stalls SET rating=?, rating_count=? WHERE id=?`).run(rating, ratingCount ?? 128, id);
        }
        catch { }
    }
}
...
function upsertMenuItem(id, stallId, catId, name, price, desc, imageUrl, rating, ratingCount) {
...
const counts = {
    ...
    boms: db.prepare(`SELECT count(*) as c FROM recipe_bom`).get().c,
