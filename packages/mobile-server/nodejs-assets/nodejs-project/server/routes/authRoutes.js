...
authRouter.post('/auth/login', (req, res) => {
    ...
    const user = db.prepare(`SELECT id, username, role, stall_id, display_name, is_active, pin FROM users WHERE username=?`).get(username);
    if (!user || user.pin !== pin)
        return res.status(401).json({ error: 'Invalid username or PIN' });
    if (!user.is_active)
        return res.status(403).json({ error: 'Account disabled' });
    // Return without pin, add stall name for convenience
    const stall = user.stall_id ? db.prepare(`SELECT name FROM stalls WHERE id=?`).get(user.stall_id) : null;
...
authRouter.get('/auth/me', (req, res) => {
    ...
    const user = db.prepare(`SELECT id, username, role, stall_id, display_name, is_active FROM users WHERE id=?`).get(userId);
    if (!user)
...
authRouter.post('/users', requireAuth, requireAdmin, (req, res) => {
    ...
    if (role === 'STALL_OWNER' && !stallId)
        return res.status(400).json({ error: 'STALL_OWNER requires stallId' });
    if (stallId) {
...
authRouter.patch('/users/:id', requireAuth, requireAdmin, (req, res) => {
    ...
    if (parsed.data.pin !== undefined) {
        fields.push('pin=?');
        vals.push(parsed.data.pin);
    }
    if (parsed.data.role !== undefined) {
        fields.push('role=?');
        vals.push(parsed.data.role);
    }
    if (parsed.data.stallId !== undefined) {
        fields.push('stall_id=?');
        vals.push(parsed.data.stallId);
    }
